...
def fitness_function(features):
    ...
    scores = cross_val_score(model, X_filcomposition_and_process, y_filtered, cv=kf, scoring='r2')
...
for generation in range(num_generations):
    ...
    fitness_scores = Parallel(n_jobs=64)(delayed(fitness_function)(individual) for individual in population)
    print(fitness_scores)

    max_r2 = pd.Series(max(fitness_scores), index=[generation])
    max_r2.to_csv('/home/wangping/3_GA/320_0.75_Tensile/max_r2(' + Model + ").csv", mode='a', header=False)
    best_individual = population[np.argmax(fitness_scores)]
    pd.DataFrame([best_individual], index=[generation]).to_csv('/home/wangping/3_GA/320_0.75_Tensile/best_individual(' + Model + ").csv", mode='a', header=False)
    ...
    selected_features.to_csv('/home/wangping/3_GA/320_0.75_Tensile/selected_features(' + Model + ").csv", mode='a', header=False)
