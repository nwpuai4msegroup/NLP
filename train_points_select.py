...
def expand_features(data, n):
    ...
    for index, row in data.iterrows():
        ...
        for j in range(len(row)):
            atom = (float(row[j])) / Elements.loc['Relative atomic mass', row.index[j]]  #Calculate the number of atoms in each element
...
data = pd.read_excel("/Users/wangping/Desktop/pythonProject/downstream task/data3.0.xlsx")
y1 = data["Yield"]
...
df = data.loc[intersection_list, :].iloc[:, 0:13]
df = expand_features(df, 13)#13
#df = df.drop_duplicates()
print('Sample size before screening of d electronic theory: (all have yield elongation, containing the same components but different processes)', df.shape[0])
inter = filter(df)
inter_diffcom = inter.drop_duplicates()
print('Sample size after screening by d electronic theory: (yield elongation rates are available, with the same composition but different processes)', inter.shape[0])
print("Containing different components", inter_diffcom.shape[0])

performance_and_process = data.iloc[inter.index]
performance = performance_and_process[["Yield", "Elongation"]]
process = data.iloc[inter.index, 13:35]
performance_and_process = pd.concat([performance, process], axis=1)
inter = pd.concat([inter, performance_and_process], axis=1)
inter.to_csv('/Users/wangping/Desktop/pythonProject/D-electron/pareto/selected_train_points.csv')
inter_diffcom.to_csv("/Users/wangping/Desktop/pythonProject/D-electron/pareto/selected_train_points_diffcom.csv")
